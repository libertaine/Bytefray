"""Thin V6 E5 runner over the consolidated experiment harness.

Defines -- and never automatically executes -- the frozen E5 matrix
(``matrix.py``): conditions C-E5 / T-E5 / C-E5K1 / T-E5K1, fields F1 / F2,
seeds 1..32, both orientations, arena 512, 1000 ticks.

Subcommands (``python -m tools.research.v6.e5.run_e5 ...``):
...
* ``plan`` (default): dry run. Builds every evaluation request of every
  ...
  evaluation planner (7,168). Executes nothing.
* ``execute CONDITION FIELD --confirm-matrix-execution``: runs one
  ...
  clean tree and the frozen engine and tooling source. A treatment (T-E5,
  T-E5K1) also needs ``--confirm-treatment-execution`` and the whole unlock
  chain (``treatment_unlock``).
* ``telemetry CORPUS``: E5 telemetry (E3 telemetry, E4 metrics and E5 metrics)
  for a control (``C-E5``, ``C-E5K1``) or a historical parent corpus
  (``historical-C-E4``, ``historical-C-E4K1``, the E5-field cells only).
* ``reproduce``: the parent reproduction gate (every re-run control field
  against the preserved C-E4 / C-E4K1 cells) and the twin-mirror relabel gate.
...
* ``populations``: freezes P-BASE, the contest classes, P-PAR-E5, the
  control-vs-control census, the control-side E5-D clauses and O-MIN-SB.
* ``p5``, ``observation``, ``d9``: the non-matrix pre-treatment gates.
...
from tools.research.v6.e4.analyze_e4 import FieldRun, make_field_run
from tools.research.v6.e5 import matrix, nonmatrix_gates, populations
from tools.research.v6.e5.analysis_freeze import (
    FREEZE_RECORD_PATH,
    AnalysisFreezeError,
    git_text,
    load_freeze,
...
from tools.research.v6.e5.analyze_e5 import E5_ANALYSIS_VERSION, evaluate_hypotheses
from tools.research.v6.e5.gates import (
    TreatmentGateError,
    compare_parent_subset,
    corpus_integrity,
    d9_completions,
    decoupling_cell_checks,
    decoupling_contest_check,
    field_keys,
    field_orientations,
    manipulation_checks,
    record_sha256,
...
from tools.research.v6.e5.preregistration import load_preregistration, preregistration_digest
from tools.research.v6.e5.telemetry import (
    compute_telemetry,
    read_rows,
    row_digests,
    summarize,
...
E5_RUNNER_VERSION = 1
DEFAULT_RUN_ROOT = REPO_ROOT / "runs" / "research_v6_e5"
HISTORICAL_RUN_ROOT = REPO_ROOT / "runs" / "research_v6_e4"
...
class E5ConfigurationError(RuntimeError):
    """An E5 run would not match the frozen experiment definition."""
...
def historical_root(condition_id: str, field_id: str, root: Path = HISTORICAL_RUN_ROOT) -> Path:
    """The preserved E4 control corpus a re-run control reproduces (C-E5 <- C-E4, C-E5K1 <- C-E4K1)."""
...
def treatment_artifacts(run_root: Path) -> list[str]:
    ...
    if records.is_dir():
        ...
        found += [str(p) for p in records.glob("*/treatment_gates_*") if p.exists()]
    return sorted(found)
...
def experiment_config(
    ...
    *,
    ...
    return ResearchExperimentConfig(
        ...
        provenance_extra={
            "e5_matrix_id": matrix.matrix_id(),
            "e5_matrix_digest": matrix.E5_MATRIX_DIGEST,
            "e5_preregistration_sha256": preregistration_digest(),
            "e5_runner_version": E5_RUNNER_VERSION,
            "e5_condition": condition_id,
            "e5_field": field_id,
            "e5_freeze_id": freeze_id,
        },
    )
...
def check_requests(requests: Sequence[EvaluationRequest], condition_id: str, field_id: str, *,
    ...
    if problems:
        raise E5ConfigurationError("E5 request check failed: " + "; ".join(problems))
...
def verify_agent_fingerprints(agents: Sequence[str]) -> dict[str, str]:
    ...
    if drift:
        raise E5ConfigurationError(f"E5 agent fingerprints differ from the frozen definition: {drift}")
    return live
...
def dry_run_plan(condition_id: str, field_id: str) -> dict[str, Any]:
    ...
    field = matrix.field(field_id)
    with tempfile.TemporaryDirectory(prefix="e5-plan-") as tmp:
        config = experiment_config(condition_id, field_id, output_dir=Path(tmp) / "out")
        if experiment_pairs(config) != field.pairs:
            raise E5ConfigurationError(f"{field_id}: planned pairs differ from the frozen field")
...
def plan_report() -> dict[str, Any]:
    ...
    return {
        ...
        "all_consistent": all(row["consistent"] for row in rows) and planned == matrix.matches_total() == 7168,
    }
...
def _run(config: ResearchExperimentConfig, condition_id: str, field_id: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="e5-guard-") as tmp:
    ...
    if cells != expected:
        raise E5ConfigurationError(f"{condition_id}/{field_id}: {cells} cells ran, {expected} expected")
...
def execute(condition_id: str, field_id: str, *, run_root: Path | None = None, confirm: bool = False,
    ...
    if not confirm:
        raise E5ConfigurationError("Refusing to execute: pass confirm=True / --confirm-matrix-execution.")
    condition = matrix.condition(condition_id)
    if condition.role == matrix.TREATMENT and not confirm_treatment:
        raise E5ConfigurationError(
    ...
    if config.output_dir.exists():
        raise E5ConfigurationError(f"{config.output_dir} already exists; an E5 cell set is never re-run in place.")
...
def corpus_roots(corpus: str, run_root: Path) -> dict[str, Path]:
    ...
    raise E5ConfigurationError(f"Unknown or non-control corpus {corpus!r}.")


def corpus_keys(corpus: str, field_id: str, run_root: Path) -> set[str] | None:
    """A historical corpus is analyzed on the E5-field cells only (the matching control's cells)."""
    if corpus in HISTORICAL_CORPORA:
...
def reproduce(*, run_root: Path | None = None, freeze_path: Path = FREEZE_RECORD_PATH) -> dict[str, Any]:
    """The parent reproduction gate and the relabel gate, recorded under the freeze."""
    root = run_root or DEFAULT_RUN_ROOT
    assert_no_treatment_artifacts(root)
    verify_frozen_definition()
    freeze = load_freeze(freeze_path)
    ...
    for condition_id, historical in matrix.HISTORICAL_PARENT.items():
        ...
        for field in matrix.FIELDS:
            ...
            if field.field_id in matrix.RELABEL_FIELDS:
                comparison["relabel"] = relabel_gate(condition_root(root, condition_id, field.field_id))
            checks = [comparison["integrity"]["status"], "PASS" if comparison["clean_telemetry"] else "FAIL",
                      comparison.get("relabel", {}).get("status", "PASS")]
    ...
    return write_parent_record(
        ...
        provenance={**get_git_provenance(), "e5_runner_version": E5_RUNNER_VERSION},
    )
...
def qualify(*, run_root: Path | None = None, workers: int = 1, freeze_path: Path = FREEZE_RECORD_PATH) -> dict[str, Any]:
    ...
    names = ("analyzed", "analyzer_failures", "cpu_statistics_mismatches", "ownership_reconstruction_disagreements",
             ...
             "e5_check_failures", "decided_early_cells")
    ...
    record = {
        ...
        "note": "Control data only: no T-E5 or T-E5K1 match exists.", "corpora": corpora, "totals": totals,
...
def require_qualification(path: Path, *, freeze_id: str) -> dict[str, Any]:
    if not path.is_file():
        raise E5ConfigurationError(f"No analyzer qualification record at {path}.")
    ...
    if (record.get("freeze_id") != freeze_id or record.get("status") != "PASS" or record.get("complete") is not True
        ...
        raise E5ConfigurationError("Analyzer qualification does not permit treatment.")
    return record


# ---------------------------------------------------------------------------
# Populations, the non-matrix gates and the treatment unlock
# ---------------------------------------------------------------------------
...
def compute_populations(*, run_root: Path, freeze_id: str) -> dict[str, Any]:
    return populations.build_record(
        freeze_id=freeze_id, preregistration_sha256=preregistration_digest(),
...
def freeze_populations(*, run_root: Path | None = None, freeze_path: Path = FREEZE_RECORD_PATH,
    ...
    return {"path": str(out_path), "sha256": sha,
            ...
            "control_d_clauses": {arm: {k: v["status"] for k, v in body["control_d_clauses"].items()}
                                  for arm, body in record["arms"].items()},
            "min_sweep_backed": record["min_sweep_backed"], "p_par_e5": record["p_par_e5"]["count"]}


GATE_RUNNERS = {
    "p5": (nonmatrix_gates.INERTNESS_RECORD_NAME, nonmatrix_gates.run_inertness),
    "observation": (nonmatrix_gates.OBSERVATION_RECORD_NAME, nonmatrix_gates.run_observation),
    "d9": (nonmatrix_gates.D9_RECORD_NAME, nonmatrix_gates.run_d9),
}


def run_nonmatrix_gate(name: str, *, run_root: Path | None = None, freeze_path: Path = FREEZE_RECORD_PATH
                       ) -> dict[str, Any]:
    root = run_root or DEFAULT_RUN_ROOT
    assert_no_treatment_artifacts(root)
    freeze = load_freeze(freeze_path)
    record_name, runner = GATE_RUNNERS[name]
    return nonmatrix_gates.write_record(freeze_root(root, freeze["freeze_id"]) / record_name, runner(),
...
CONTROL_RECORDS: tuple[str, ...] = (PARENT_RECORD_NAME, QUALIFICATION_RECORD_NAME,
                                    *(record for record, _ in GATE_RUNNERS.values()))
...
def treatment_unlock(run_root: Path, *, freeze_path: Path = FREEZE_RECORD_PATH,
    ...
    qualification names passing, complete records whose bytes are unchanged, the
    frozen populations recompute from the control corpus, and none of their hard
    stops (control census, control-side E5-D, O-MIN-SB) fires."""
    freeze = load_freeze(freeze_path)
    freeze_id = freeze["freeze_id"]
    ...
    require_relabel({f"{c}/{f}": parent["reproduction"][c][f]["relabel"]
                     for c in matrix.CONTROL_CONDITIONS for f in matrix.RELABEL_FIELDS})
    analyzer = require_qualification(records / QUALIFICATION_RECORD_NAME, freeze_id=freeze_id)
    gates = {name: nonmatrix_gates.require_gate(records / record, freeze_id=freeze_id)
             for name, (record, _) in GATE_RUNNERS.items()}
    ...
    return {"freeze": freeze, "parent": parent, "analyzer": analyzer, "gates": gates, "populations": frozen}
...
ANALYSIS_RECORD_NAME = "e5_analysis.json"
...
def write_treatment_telemetry(condition_id: str, *, run_root: Path | None = None, workers: int = 1,
    ...
    if matrix.condition(condition_id).role != matrix.TREATMENT:
        raise E5ConfigurationError(f"{condition_id} is not a treatment.")
    root = run_root or DEFAULT_RUN_ROOT
...
def treatment_gate_report(condition_id: str, *, treatment_roots: dict[str, Path],
                          rows: dict[str, dict[str, dict[str, Any]]], frozen_arm: dict[str, Any],
                          integrity: dict[str, dict[str, Any]] | None = None) -> dict[str, Any]:
    """The treatment hard stops for one treatment: integrity, manipulation checks,
    E5-D (D-1 .. D-4 per cell, D-5 per ANCHOR-ONLY unit), the relabel gate and D9."""
    fields: dict[str, Any] = {}
    completions: list[dict[str, Any]] = []
    for field_id in matrix.FIELD_IDS:
        completions += d9_completions(rows[field_id], field_id)
        report: dict[str, Any] = {"manipulation": manipulation_checks(rows[field_id]),
                                  "decoupling": decoupling_cell_checks(rows[field_id])}
        if field_id in matrix.RELABEL_FIELDS:
            report["relabel"] = relabel_gate(treatment_roots[field_id])
        if integrity is not None:
            report["integrity"] = integrity[field_id]
        checks = [report["manipulation"]["status"], report["decoupling"]["status"],
                  report.get("relabel", {}).get("status", "PASS"), (integrity or {}).get(field_id, {}).get("status", "PASS")]
        report["status"] = "PASS" if all(status == "PASS" for status in checks) else "FAIL"
        fields[field_id] = report
    contest = decoupling_contest_check(rows, frozen_arm["units"], frozen_arm["p_base"], frozen_arm["contest_classes"])
    e5_d = "PASS" if contest["status"] == "PASS" and all(r["decoupling"]["status"] == "PASS"
                                                         for r in fields.values()) else "FAIL"
    ...
    return {"condition": condition_id, "parent": matrix.condition(condition_id).parent, "fields": fields,
            "decoupling_contests": contest, "e5_d": e5_d, "d9_completions": completions[:20],
            "d9_completion_count": len(completions), "d9_stop": d9_stop,
            "status": "PASS" if all(r["status"] == "PASS" for r in fields.values()) and e5_d == "PASS"
            and not d9_stop else "FAIL"}
...
def treatment_gates(condition_id: str, *, run_root: Path | None = None,
    ...
    record = treatment_gate_report(
        ...
        frozen_arm=unlock["populations"]["arms"][condition.arm], integrity=integrity)
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(record, indent=2, sort_keys=True), encoding="utf-8")
    require_d9(condition_id, record["d9_completions"])
    require_decoupling({"status": record["e5_d"]})
...
def analyze(*, run_root: Path | None = None, freeze_path: Path = FREEZE_RECORD_PATH) -> dict[str, Any]:
    ...
    result = evaluate_hypotheses(arms, unlock["populations"], load_preregistration(),
                                 e4_contest_classes.load_table(), d_status=d_status)
    record = {"e5_analysis_version": E5_ANALYSIS_VERSION, "matrix_id": matrix.matrix_id(), "freeze_id": freeze_id,
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="run_e5", description=(__doc__ or "").splitlines()[0])
    ...
    tele = sub.add_parser("telemetry", help="E5 telemetry for a control or historical parent corpus")
    ...
    for name, text in (("reproduce", "parent reproduction and the relabel gate"),
                       ("populations", "freeze the control populations"),
                       ("p5", "the P5 inertness gate"), ("observation", "the observation-delta gate"),
    ...
    if args.command == "populations":
        ...
        return 0
    if args.command in GATE_RUNNERS:
        record = run_nonmatrix_gate(args.command, run_root=args.run_root)
        emit({k: v for k, v in record.items() if k not in ("rows", "scenarios", "primary_failures")})
        return 0 if record["status"] == "PASS" else 1
