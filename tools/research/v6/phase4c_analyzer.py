"""V6 Phase 4C movement normalization study -- post-hoc analysis.

Reads the evaluation artifacts ``phase4c_corpus_runner.py`` wrote
(``runs/research_v6_phase4c/{equivalence,sweep}/...``) and computes the
...
def summarize_condition(cells: list[dict[str, Any]], field: list[str], arena_size: int) -> dict[str, Any]:
    ...
    return {
        "arena_size": arena_size,
        "max_move_delta": max(64, arena_size // 8),
...
def main() -> int:
    conditions = {
        512: RUNS_ROOT / "equivalence" / "research-scale-move",
    ...
    # Comparative analysis against Phase 4B if available
    ...
    if p4b_analysis_path.exists():
        ...
        for a_size, s4c in summaries.items():
            if a_size in p4b_summaries:
                ...
                win_rate_deltas = {agent: w4c[agent] - w4b[agent] for agent in FIELD}
                rho_cross_phase = spearman_rho(r4b, r4c, list(FIELD))

                t2_4b = s4b.get("tier2") or {}
                t2_4c = s4c.get("tier2") or {}

                comparative[str(a_size)] = {
                    "spearman_rho_phase4c_vs_phase4b": rho_cross_phase,
                    "win_rate_deltas_4c_minus_4b": win_rate_deltas,
                    "timeout_rate_delta": (s4c["timeout_rate"] or 0) - (s4b["timeout_rate"] or 0),
                    "mean_ticks_delta": (s4c["mean_ticks_run"] or 0) - (s4b["mean_ticks_run"] or 0),
                    ...
                    "tier2_deltas": {
                        "mean_contact_delta": (
                            ...
                            if t2_4c.get("mean_first_contact_tick") and t2_4b.get("mean_first_contact_tick")
                            else None
                        ),
                        "never_contact_rate_delta": (
                            (t2_4c.get("matches_never_in_contact_rate") or 0)
                            - (t2_4b.get("matches_never_in_contact_rate") or 0)
                            if t2_4c.get("matches_never_in_contact_rate") is not None
                            else None
                        ),
                        "stagnation_rate_delta": (
                            (t2_4c.get("stagnation_rate") or 0) - (t2_4b.get("stagnation_rate") or 0)
                            if t2_4c.get("stagnation_rate") is not None
                            else None
                        ),
                    },
                }
    ...
    output_payload = {
        "phase4c_summaries": {str(k): v for k, v in summaries.items()},
        "comparative_vs_phase4b": comparative,
    }

    out_path = RUNS_ROOT / "phase4c_analysis.json"
    out_path.write_text(json.dumps(output_payload, indent=2, default=str), encoding="utf-8")
    print(f"\nwritten: {out_path}")

    for arena_size, summary in summaries.items():
        print(f"\n=== arena {arena_size} (max_move_delta={summary['max_move_delta']}) ===")
        ...
        if str(arena_size) in comparative:
            c = comparative[str(arena_size)]
            print(f"  [vs Phase 4B raw] rho={c['spearman_rho_phase4c_vs_phase4b']:.3f}, timeout_delta={c['timeout_rate_delta']:+.1%}")
