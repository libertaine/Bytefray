...
| **Next Research Phase** | E3 — Slot-Limited Disruption | **Tooling frozen; controls qualified; treatment not run** | Two research Rulesets bound disruption to the victim's next action offer instead of the rest of the tick: the primary `bytefray-rules-6-research-capture-hold-k2-disruption-slot1` (parent E2) and the companion `bytefray-rules-6-research-disruption-slot1` (parent research-scale). Both are implemented and qualified. The research tooling, the D0–D9 pre-registration, matrix `v6-e3-matrix-v1-634132ec3c15` (19,456 matches) and analysis freeze `v6-e3-freeze-v1-506811e78ad8` are frozen. Both controls (C-E2, C-RS) have been run and reproduce their historical E2 parents cell for cell; the analyzer, the D9 real-fixture gate and the control populations are qualified on control data only. Neither treatment has been run, and no gameplay result exists. See [experiment freeze](research/v6/V6_E3_EXPERIMENT_FREEZE.md) and [registration](research/v6/V6_E3_SLOT_LIMITED_DISRUPTION_REGISTRATION.md). |
