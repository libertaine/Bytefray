...
| **Immediate Next Phase** | E2 — Multi-Tick Capture Hold | **Planned** | Research Ruleset `bytefray-rules-6-research-capture-hold-k2` implemented and qualified; research fixtures, capture analyzer and harness remediation in place and the experiment definition frozen. Matrix execution halted before the treatment: controls ran and the control gate passed, but a capture-analyzer defect requires re-qualification and a re-freeze. Asks whether a one-tick capture hold creates opponent-dependent response or only delay, scheduler-locked draws, or another seat pathology. |
