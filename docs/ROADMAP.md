...
| **Research Phase After E4** | E5 — Anchor/Core-0 Separation | **Tooling frozen; controls qualified; treatment not run** | A research question, not a promised improvement: Is anchor/core-0 co-location necessary for the second mover's directed base privilege, where the base is also attacked explicitly? Two research Rulesets spawn every process with no declared position one cell before its entrant's core (`(core_base - 1) % arena_size`) instead of on core cell 0, each differing from its E3 parent only in `initial_anchor_placement = "before_core"`: `bytefray-rules-6-research-capture-hold-k2-disruption-slot1-anchor-before-core` (primary) and `bytefray-rules-6-research-disruption-slot1-anchor-before-core` (companion). A spawn rule only: movement is unrestricted. Both are implemented and qualified as research-only. The E5 research tooling, the E5-D, E5-H1–H5 and D9 pre-registration, matrix `v6-e5-matrix-v1-ef7fa327ea81` (7,168 matches) and analysis freeze `v6-e5-freeze-v1-5ba12be258c8` are frozen. Both controls (C-E5, C-E5K1) have been run and reproduce the preserved E4 C-E4 and C-E4K1 corpora cell for cell; the analyzer, the P5 inertness, observation-delta and D9 gates and the control populations (17 sweep-backed units in the primary arm, at least 6 required) are qualified on control data only. Neither treatment has been run, and no gameplay result exists. See [experiment freeze](research/v6/V6_E5_EXPERIMENT_FREEZE.md), [design review](research/v6/V6_E5_ANCHOR_CORE_SEPARATION_DESIGN_REVIEW.md), [Revision 1](research/v6/V6_E5_DESIGN_REVIEW_REVISION_1.md) and [registration](research/v6/V6_E5_ANCHOR_CORE_SEPARATION_REGISTRATION.md). |
