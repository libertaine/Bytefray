...
| **Next Research Phase** | E3 — Slot-Limited Disruption | **Implemented; matrix not run** | Two research Rulesets bound disruption to the victim's next action offer instead of the rest of the tick. The primary treatment is `bytefray-rules-6-research-capture-hold-k2-disruption-slot1`, whose parent is E2. The companion is `bytefray-rules-6-research-disruption-slot1`, whose parent is research-scale. Both are implemented and qualified as research-only. No E3 experiment matrix has been run, and no gameplay result exists. See [registration](research/v6/V6_E3_SLOT_LIMITED_DISRUPTION_REGISTRATION.md). |
